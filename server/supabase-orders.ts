...
export async function createSupabaseOrder(payload: {
  ...
  const menuItems = await request<MenuItemRow[]>(
    `menu_items?id=in.(${ids.map(encodeURIComponent).join(',')})&select=id,name,price_kes,is_available`
  );
  ...
  let subtotal = 0;
  const orderItems = [] as Array<Record<string, unknown>>;
  for (const requested of payload.items) {
    ...
    orderItems.push({
      ...
      unit_price: Number(menuItem.price_kes),
      subtotal: lineTotal,
      created_at: now,
    });
  }
  ...
  try {
    ...
    deliveryFee = payload.order_type === 'delivery' ? 150 : 0;
  }
  ...
  try {
    ...
    // Best-effort compensation so a partially-created order is not left behind.
