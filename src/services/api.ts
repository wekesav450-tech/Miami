...
import {
  MenuCategory, MenuItem, Order, Reservation, UserProfile, CreateOrderPayload,
  CreateReservationPayload, AdminStats, PaymentRecord,
} from '../types';
...
export const authStorage = {
  ...
  getProfile(): UserProfile | null { const raw = localStorage.getItem(USER_KEY); if (!raw) return null; try { return JSON.parse(raw); } catch { return null; } },
...
async function apiRequest<T>(endpoint: string, options: RequestInit = {}): Promise<T> {
  ...
  if (!response.ok) {
    const message = typeof data.detail === 'string' && data.detail.trim() ? `${data.error || 'Request failed'} — ${data.detail}` : data.error || `Request failed with status ${response.status}`;
...
export const api = {
  ...
  menu: {
    ...
    async updateItem(id: string, updates: Partial<Pick<MenuItem, 'is_available' | 'price_kes' | 'name' | 'description' | 'is_featured'>>): Promise<MenuItem> { const res = await apiRequest<{ item: MenuItem }>(`/api/admin/menu/items/${id}`, { method: 'PATCH', body: JSON.stringify(updates) }); return res.item; },
  },
  orders: {
  ...
  realtime: { connect(_onMessage: (event: { type: string; data: any; timestamp: string }) => void): () => void { return () => {}; } },
};
